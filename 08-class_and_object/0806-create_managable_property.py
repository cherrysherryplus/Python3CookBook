...
class Person:
    def __init__(self, first_name) -> None:
        ...
        #  另外，还可以在 __init__() 方法中直接设置 self.first_name 而不是 self._first_name 。 
        # 在这个例子中，我们创建一个property的目的就是在设置attribute的时候进行检查。 
        # 因此，你可能想在初始化的时候也进行这种类型检查。通过设置 self.first_name ，自动调用 setter 方法， 
        # 在setter方法里面会进行参数的检查，否则就是直接访问 self._first_name 了。
        # 如下：
    ...
    def first_name(self, value):
        # 下面这一个if语句,验证了在__init__中直接设置property的属性值,会调用setter函数
        if not hasattr(self, '_first_name'):
            print('setter are invoked in __init__')
...
# 但是访问它的时候会自动触发 getter 、setter 和 deleter 方法。
# 例如：
a = Person('Guido')
# Calls the getter
print(a.first_name)
...
# 如果你去查看拥有property的类， 就会发现property本身的fget、fset和fdel属性就是类里面的普通方法。比如：
...
# 当你以后想给普通attribute访问添加额外的处理逻辑的时候， 
...
# 重复代码会导致臃肿、易出错和丑陋的程序。
# 好消息是，通过使用装饰器或闭包，有很多种更好的方法来完成同样的事情。 可以参考8.9和9.21小节的内容。
